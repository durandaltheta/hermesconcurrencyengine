...
#ifndef HCESCHEDULERALLOCATORDEFAULTSIZELIMIT
#define HCESCHEDULERALLOCATORDEFAULTSIZELIMIT 1024
#endif
        ...
        hce::lockfree>
{
    ...
    static inline std::string info_name() { 
        return type::templatize<T>("hce::detail::scheduler::sync_partial"); 
    }

    inline std::string name() const { return sync_partial<T>::info_name(); }
        ...
        hce::awaitable::lockable<hce::awt_interface<void>,hce::lockfree>
{
    ...
    static inline std::string info_name() { 
        return type::templatize<void>("hce::detail::scheduler::sync_partial"); 
    }

    inline std::string name() const { return sync_partial<void>::info_name(); }
        ...
        hce::spinlock>
{
    ...
    static inline std::string info_name() { 
        return type::templatize<T>("hce::detail::scheduler::async_partial"); 
    }

    inline std::string name() const { return async_partial<T>::info_name(); }
        ...
        hce::spinlock>
{
    ...
    static inline std::string info_name() { 
        return type::templatize<void>("hce::detail::scheduler::async_partial"); 
    }

    inline std::string name() const { return async_partial<void>::info_name(); }
...
struct scheduler : public printable {
    static constexpr size_t pool_default_size_limit = 
        HCESCHEDULERALLOCATORDEFAULTSIZELIMIT;
    ...
    struct lifecycle : public printable {
        ...
        struct manager : public printable {
            virtual ~manager() { HCE_HIGH_DESTRUCTOR(); }
            ...
            /**
             @brief register a lifecycle pointer to be destroyed at process exits

             It is expected that the scheduler is running before registration
             */
            ...
            /**
             @brief register an awaitable interface for the manager join eventually

             Detached awaitables are handled by this particular process-wide 
             mechanism. This is useful because it allows for agnostic 
             synchronization of launched operations that are guaranteed to 
             complete before the global scheduler is permanently shutdown.

             That is, the user can complete all their operations, and even exit 
             main(), and awaitables passed to this mechanism will still complete 
             successfully. It is used by hce::awaitable::detach().

             WARNING: interfaces registered with this mechanism are only 
             guaranteed to be joined *on process exit*. It is important that 
             done_await is called when the operation is complete to free memory.

             @param i an awaitable::interface that needs to be awaited
             @return a key to pass to done_await()
             */
            size_t register_await(awaitable::interface* i);

            /**
             @brief notify the manager a registered awaitable interface is ready to join

             It is important for completed awaitable interfaces which previously 
             were registered with register_await() to call this method to inform 
             the manager that they can be joined and cleaned up immediately. 

             Without calling this interface will *eventually* be joined at 
             process exit, but the manager will otherwise hold onto the pointer 
             until then.

             @param key the key returned from register_await()
             */
            void done_await(size_t key);
        ...
        private:
            manager() : state_(executing) { 
                ...
                // ensure this happens after constructor list because this may 
                // actually spawn and register the global scheduler with this 
                // manager
                root_await_ = scheduler::global().schedule(root_await_op_());
            }

            static inline void atexit() {
                // shutdown and join the awaiter operation before any global 
                // destructors are called
                manager::instance().shutdown_root_await_op_(); 
            }

            // coroutine to await arbitrary awaitables
            hce::co<void> root_await_op_();

            // shutdown the awaiter coroutine
            void shutdown_root_await_op_();
            ...
            std::list<std::unique_ptr<lifecycle>> lifecycle_pointers_;

            // destroy this first so we await all outstanding awaitables
            hce::awt<void> root_await_;
        };
    ...
    struct config : public printable {
        ...
        static inline std::string info_name() { 
            return "hce::scheduler::config"; 
        }

        inline std::string name() const { return config::info_name(); }
        ...
        */
        ...
        inline std::string content() const {
            ...
            ss << "log_level:"
               << log_level
               << ", coroutine_pool_limit: "
               << coroutine_pool_limit
               << ", block_worker_pool_limit: "
               << block_worker_pool_limit;
         ...
         */
        size_t block_worker_pool_limit;

    private:
        config() :
            log_level(hce::printable::default_log_level()),
            coroutine_pool_limit(scheduler::pool_default_size_limit),
            block_worker_pool_limit(0)
        { 
            HCE_HIGH_CONSTRUCTOR(); 
        }
    };
    ...
    /**
     ...
     Allocates and registers a new `scheduler` with the `lifecycle::manager` 
     instance to be  when the process exits.
     ...
     Unless the user wishes to manage a custom threadpool of schedulers, it is 
     unlikely they will ever need to use this function directly. The `global()`
     scheduler and default threadpool take care of calling this function and 
     registering the `lifecycle` with the `lifecycle::manager`. 

     If the user needs to create and manage a scheduler:
     ```
     // create a new scheduler and launch it on a new thread
     std::unique_ptr<hce::scheduler::lifecycle> lf = hce::scheduler::make();

     // acquire the scheduler's shared pointer
     std::shared_ptr<hce::scheduler> sch = lf->scheduler();

     // ... later 
     
     // scheduler runs until halted by ~lifecycle
     lf.reset() 
     ```
    ...
    static inline std::unique_ptr<lifecycle> make(
        ...
        return std::unique_ptr<lifecycle>(new lifecycle(std::move(s)));
    }
    ...
    /**
     Useful heuristic for determining ideal `block_worker_pool_limit()` size.
    ...
    /**
     This value is determined by the `scheduler::config::coroutine_pool_limit` 
     ...
     A `coroutine_pool_limit()` at or above the median count of coroutines 
     operating in user code is often a sensible value.
     */
    inline size_t coroutine_pool_limit() const {
        HCE_MIN_METHOD_BODY("coroutine_pool_limit",coroutine_pool_limit_);
        return coroutine_pool_limit_;
    }

    /**
     This value is determined by the `
     scheduler::config::block_worker_pool_limit` member in the 
     ...
     */
    inline size_t block_worker_pool_limit() const { 
        HCE_MIN_METHOD_BODY("block_worker_pool_limit",block_worker_pool_->size());
    ...
    /**
     ...
     `block()`'s potential failure behavior is identical to `schedule()`. `block()` 
     calls act as a scheduled operation and therefore block `lifecycle` object's
     destructor from  a `scheduler` until they return.
...
private:
    ...
    void configure_(std::shared_ptr<scheduler>& self, 
        ...
        coroutine_pool_limit_ = cfg->coroutine_pool_limit;
        coroutine_queue_.reset(new hce::queue<std::coroutine_handle<>>(
            coroutine_pool_limit_));
        block_worker_pool_.reset(
            new hce::circular_buffer<std::unique_ptr<blocking::worker>>(
                cfg->block_worker_pool_limit));
    }
    ...
    void run() {
        ...
        // assign the thread_local scheduler pointer to the this scheduler
        detail::scheduler::tl_this_scheduler() = this;

        // the local queue of coroutines to evaluate
        std::unique_ptr<hce::queue<std::coroutine_handle<>>> local_queue(
            new hce::queue<std::coroutine_handle<>>(
                coroutine_pool_limit_));
    ...
    // of scheduler::run().
    std::unique_ptr<hce::queue<std::coroutine_handle<>>> 
    coroutine_queue_;
    ...
    // config::block_worker_pool_limit. 
