...
/**
 ...
 the scheduler during its runtime with the given options.
...
struct scheduler : public printable {
    ...
    struct reschedule : public INTERFACE {
        ...
        inline void on_suspend() {
            /* 
             Now that we're suspending we acquire a destination scheduler. If 
             we're not in a scheduler then we're going to block on a
             `thread_local` condition variable and `to_destination()` won't be 
             called.
             */
        ...
        inline void to_destination(std::coroutine_handle<> h) {
            ...
            // call private schedule method to skip unnecessary handle 
            // validation, and because no joiner awaitable needs to be made
    ...
    struct lifecycle : public printable {
        /**
         @brief process-wide instanced object responsible for managing scheduler lifecycles
         */
    ...
    /**
     ...
     The global scheduler is always the first scheduler in the hce::threadpool 
     (index 0). It is also always guaranteed to exist.
     */
    ...
    inline hce::awt<T> schedule(hce::co<T> co) {
        ...
        // the awaitable allowing joining with the scheduled coroutine
        hce::awt<T> awt;
