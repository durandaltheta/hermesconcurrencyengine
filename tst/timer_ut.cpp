...
// 1/10th of a second
const unsigned int milli_one_tenth_second(100); 
...
void validate_test(std::optional<double> check_sleep, 
    ...
    if(check_running) {
        ...
        std::cout << "timer target window running check rate: " << running_success_percentage << "%" <<std::endl;
    ...
    if(check_busywait) {
        ...
        std::cout << "timer service busy-wait microsecond threshold: " << ticks.threshold << std::endl;
...
TEST_F(timer, start_medium) {
    ...
    test::timer::validate_test({95.0},{98.0},{5.0});
}
...
TEST_F(timer, start_long) {
    ...
    for(unsigned int i=1; i<test::timer::iterate_limit; ++i) {
        const unsigned int milli_dur = i * test::timer::milli_one_tenth_second;
        HCE_INFO_LOG("TEST_F(timer,start_long) milli:%u",milli_dur);
        EXPECT_EQ(expected_successes,test::timer::start(std::chrono::milliseconds(milli_dur)));
    }

    test::timer::validate_test({95.0},{98.0},{1.0});
}
...
TEST_F(timer, sleep_medium) {
    ...
    test::timer::validate_test({95.0},{},{5.0});
}
